...
namespace Meridian59.Patcher
{
   ...
   public class Sha256
   {
      ...
      private static UInt32 ROTL(UInt32 x, byte n) { return (x << n) | (x >> (32 - n)); }
      ...
      private static UInt32 Sigma0(UInt32 x) { return ROTR(x, 2) ^ ROTR(x, 13) ^ ROTR(x, 22); }
      private static UInt32 Sigma1(UInt32 x) { return ROTR(x, 6) ^ ROTR(x, 11) ^ ROTR(x, 25); }
